...
public class SearchBasedOnInterests implements PositionsSearchStrategy {
    ...
    public SearchBasedOnInterests(TraineeshipPositionDao positionDao, StudentDao studentDao) {
    ...
    public List<TraineeshipPosition> search(String applicantUsername) {
        ...
        if (studentInterests == null || studentInterests.isEmpty()) return List.of();
        ...
        for (TraineeshipPosition position : allPositions) {
            ...
            Set<String> topics = Arrays.stream(topicsRaw.split(","))
                    .map(String::trim)
                    .filter(s -> !s.isBlank())
                    .collect(Collectors.toSet());

            Set<String> interests = new HashSet<>(studentInterests);

            double similarity = calculateJaccard(interests, topics);
